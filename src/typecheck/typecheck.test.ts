...
// let x = 1 in x
test("let式 let x = 1 in x の型は TInt", () => {
	const ast: Expr = {
		type: "LET",
		name: "x",
...
test("if式 if true then 1 else 2 の型は TInt", () => {
	const ast: Expr = {
		type: "IF",
...
test("関数 let rec f x = x + 1 in f 1 の型は TInt", () => {
	const ast: Expr = {
		type: "LetRec",
		func: {
			name: "f",
...
// TODO: 再帰関数のテスト
// ===========================================
// TODO: let rec fib n = if n < 2 then n else fib (n - 1) + fib (n - 2) in fib 10

// ===========================================
// TODO: 複数引数の関数
// ===========================================
// TODO: let rec add x y = x + y in add 1 2

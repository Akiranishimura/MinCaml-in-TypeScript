...
describe("Lexer", () => {
	...
	describe("配列", () => {
		test("配列宣言をトークン化できる", () => {
			const tokens = tokenize("Array.create");
			expect(tokens).toEqual([{ type: "ARRAY_CREATE" }, { type: "EOF" }]);
		});
		test("配列アクセスをトークン化できる", () => {
		...
		test("配列代入へのアクセスをトークン化できる", () => {
			const tokens = tokenize("arr.(i) <- v");
			expect(tokens).toEqual([
